...
def save_current():
    ...
    sav_btn = Button(tl_btn_grp_frame,text="Save",command=lambda:save_file(win_display_area,win),width=15,padx=5,pady=5)
    sav_btn.grid(row=0,column=0)

    cont_btn = Button(tl_btn_grp_frame,text="Continue without saving",command=lambda:new_file(win_display_area,win),padx=5,pady=5)
    cont_btn.grid(row=0,column=1)
  

def new_file(text_area,win):
...
def delete_items(win_frame_right):
...
######################  Start Main Tk Window  ####################################
    ...
    win_submit = Button(win_frame_left,text="Save (Or press <Enter>)",command=list_update,padx=5,pady=5)
    ...
    win.bind('<Return>', lambda x:list_update())

    win_display = Button(win_frame_left,text="Show List",command=lambda:show_list(win_display_area),padx=5,pady=5)
    win_display.grid(row=6,column=0,columnspan=2,sticky=EW)

    win_display = Button(win_frame_left,text="Item Details",command=lambda:show_details(win_display_area),padx=5,pady=5)
    win_display.grid(row=7,column=0,columnspan=2,sticky=EW)

    win_display = Button(win_frame_left,text="Delete From List",command=lambda:delete_items(win_display_area),padx=5,pady=5)
    ...
    win_quit.grid(row=9,column=0,columnspan=2,sticky=EW)
